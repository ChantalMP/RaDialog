...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        # self.img_proj_layer = nn.Linear(768, config.hidden_size)
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def prepare_inputs_for_generation(
        ...
        if 'dicom' in kwargs and kwargs.get("dicom") is not None:
            dicom = kwargs.get("dicom")
            expand_size = len(input_ids) // len(dicom)
            if expand_size > 1: # this happens if they were interleaved before for something like beam search or top-k sampling. So we need to interleave dicom as well
                dicom = repeat_interleave(dicom, expand_size)
            model_inputs.update({"dicom": dicom})

        if kwargs.get("use_img") is not None:
            model_inputs.update({"use_img": kwargs.get("use_img")})
